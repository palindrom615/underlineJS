...
function play_multi_sound(s) {
  ...
  for (let a = 0; a < audiochannels.length; a++) {
    ...
    if (audiochannels[a]["finished"] < thistime.getTime()) {
      ...
      audiochannels[a]["channel"].autoplay = true;
...
var musicLevel = function(startPoint, endPoint, ratio) {
  ...
  let level = Math.floor(length / 30);
  if (level > 19) {
    level = 19;
  }
  level = 19 - level;
...
export default class GuitarString {
  constructor(ctx, startPoint, endPoint, strokeWidth, strokeColor, ratio) {
    ...
    this.canvas.addEventListener("touchmove", this.touchXY.bind(this), false);
  ...
  mouseMove(event) {
    ...
    var currentWaveDistance =
      radius -
      Math.sqrt(
        Math.pow(radius, 2) -
          Math.pow(Math.abs(this.endPoint.x - this.startPoint.x) / 2, 2)
      );
    var lastRadius = circleCenter(
      new Point(this.startPoint.x, this.startPoint.y),
      new Point(this.lastMouseX, this.lastMouseY),
      new Point(this.endPoint.x, this.endPoint.y)
    ).r;
    var lastWaveDistance =
      lastRadius -
      Math.sqrt(
        Math.pow(lastRadius, 2) -
          Math.pow(Math.abs(this.endPoint.x - this.startPoint.x) / 2, 2)
      );

    var mouseInGrabRange =
      currentWaveDistance < this.maxGrabDistance &&
      this.currentMouseX > this.startPoint.x &&
      this.currentMouseX < this.endPoint.x;

    var lastMouseOutGrabRange = !(
      lastWaveDistance < this.maxGrabDistance &&
      this.lastMouseX > this.startPoint.x &&
      this.lastMouseX < this.endPoint.x
    );

    var mouseOutControlRange = !(
      currentWaveDistance < this.maxControlDistance &&
      this.currentMouseX > this.startPoint.x &&
      this.currentMouseX < this.endPoint.x
    );

    var lastMouseInControlRange =
      lastWaveDistance < this.maxControlDistance &&
      this.lastMouseX > this.startPoint.x &&
      this.lastMouseY < this.endPoint.x;

    var mouseCrossed = intersects(
      this.lastMouseX,
      this.lastMouseY,
      this.currentMouseX,
      this.currentMouseY,
    ...
    if (mouseInGrabRange && lastMouseOutGrabRange && !this.userInControl) {
      ...
      mouseOutControlRange &&
      lastMouseInControlRange &&
      this.userInControl
    ) {
      // console.log('boing!');
      this.initState = false;
  ...
  drawArc(startPoint, thirdPoint, endPoint) {
    var ctx = this.ctx;
    ctx.lineWidth = this.strokeWidth;
    ctx.strokeStyle = this.strokeColor;

    var centerObject = circleCenter(
